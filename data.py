...
def load_medical_mnist(n_ex, data_dir='medical_mnist'):
    medical_mnist = ImageFolder(data_dir,
                                transforms.Compose([
                                   #transforms.Resize((224, 224)), 
                                   transforms.Resize((299, 299)), 
                                    transforms.ToTensor()
                                ]))
    medical_mnist_loader = DataLoader(medical_mnist, batch_size=n_ex, shuffle=True, num_workers=1)
    x_test, y_test = next(iter(medical_mnist_loader))
    print('inside medical mnist data loader:')
    return np.array(x_test, dtype=np.float32), np.array(y_test)


datasets_dict = {'mnist': load_mnist,
                 'cifar10': load_cifar10,
                 'medical_mnist': load_medical_mnist
}
